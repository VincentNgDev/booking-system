...
`;
            ...
            for (const decl of this.zmodel.declarations) {
                switch (decl.$type) {
                    ...
                    case ast_1.DataModel:
                        this.generateModel(prisma, decl, fieldsToAddGroupedByOppRelationModel[decl.name]);
                        break;
    ...
    generateModel(prisma, decl, oneSidedRefRelationFields) {
        ...
        if (oneSidedRefRelationFields) {
            oneSidedRefRelationFields.forEach((field) => {
                if (decl.fields.map(f => { var _a, _b; return (_b = (_a = f.type.reference) === null || _a === void 0 ? void 0 : _a.ref) === null || _b === void 0 ? void 0 : _b.name; }).includes(field.relationModelType)) {
    ...
    getRemoveColumn(decl) {
        const attrRemoveColumn = (0, sdk_2.getAttribute)(decl, '@@removeColumn');
        if (attrRemoveColumn !== undefined) {
            const removeColumnArg = (0, sdk_2.getAttributeArg)(attrRemoveColumn, 'column');
            if (removeColumnArg && (0, ast_1.isArrayExpr)(removeColumnArg)) {
                return removeColumnArg.items
                    .filter((item) => (0, ast_1.isStringLiteral)(item))
                    .map((item) => item.value);
            }
        }
        return [];
    }
    isFieldToRetain(field, removedColumns) {
        var _a;
        const isJoiningField = (0, sdk_1.hasAttribute)(field, '@relation') || (0, ast_1.isDataModel)((_a = field.type.reference) === null || _a === void 0 ? void 0 : _a.ref);
        const isNotInRemovedColumns = !removedColumns.includes(field.name);
        return isJoiningField || isNotInRemovedColumns;
    }
    computeRelationOneSidedRef(zmodel) {
        // Get all data models
        const models = zmodel.declarations.filter((decl) => decl.$type === ast_1.DataModel);
        const fieldsToAdd = [];
        // Generate the opposite fields of one-sided reference relations
        models.forEach((model) => {
            const oneSidedRefRelations = model.fields.filter((field) => {
                var _a;
                return (0, ast_1.isDataModel)((_a = field.type.reference) === null || _a === void 0 ? void 0 : _a.ref) &&
                    (0, sdk_1.hasAttribute)(field, '@relation') &&
                    (0, sdk_1.hasAttribute)(field, '@relationOneSideRef');
            });
            const oppositeFieldToAddOfThisModel = oneSidedRefRelations
                .map((dmf) => {
                var _a, _b;
                const oneSidedRefAttr = (0, sdk_2.getAttribute)(dmf, '@relationOneSideRef');
                if (!oneSidedRefAttr) {
                    return;
                }
                /**
                ...
                // Opposite model type
                const oppFieldModelType = (_b = (_a = dmf.type.reference) === null || _a === void 0 ? void 0 : _a.ref) === null || _b === void 0 ? void 0 : _b.name;
                // Relation Type arg from the attribute e.g. OneToOne, OneToMany
                const relationTypeArg = (0, sdk_2.getAttributeArg)(oneSidedRefAttr, 'relationType');
                // Opposite field name arg from the attribute e.g. seed
                const oppositeFieldNameArg = (0, sdk_2.getAttributeArg)(oneSidedRefAttr, 'oppositeFieldName');
                let isArray = false;
                let isOptional = true;
                ...
                if ((0, ast_1.isStringLiteral)(relationTypeArg)) {
                ...
                if ((0, ast_1.isStringLiteral)(oppositeFieldNameArg) && oppositeFieldNameArg.value.length > 0) {
                    oppModelFieldName = oppositeFieldNameArg.value;
                }
                // Prisma model field type of opposite field
                const oppModelFieldType = new prisma_builder_1.ModelFieldType(thisModel, isArray, isOptional);
                return {
    ...
    generateRelationOneSidedRefModelField(model, fieldName, prismaModelFieldType) {
